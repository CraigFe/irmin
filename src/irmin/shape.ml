  ...
  struct
    ...
    module Types = struct
      ...
      and ('variant, 'pat, 'pat_nil, 'paths, 'paths_nil) cases =
        | Cases_nil
            : ('variant, 'pat_nil, 'pat_nil, 'paths_nil, 'paths_nil) cases
        | Cases_cons :
      ...
      and 'a boxed_typ = Boxed_typ : ('a, 'a_conc) typ -> 'a boxed_typ

      and (_, _) path =
        | Path : {
            p_project_type : 's boxed_typ -> 'a boxed_typ;
            p_steps : step list;
            p_get : deref:Addr.deref -> 's -> 'a option Lwt.t;
            p_update :
              deref:Addr.deref -> pure:Addr.pure -> 's -> 'a -> 's Lwt.t;
          }
            -> ('s, 'a) path
    end
    ...
    let tree_concrete_t elt_t =
      ...
      mu (fun tree_concrete_t ->
          ...
          |~ case1 "branch" (assoc_concrete_t tree_concrete_t) (fun b ->
                 Branch b)
    ...
    module Path = struct
      ...
      let get ~deref (Path { p_get; _ }) = p_get ~deref

      let update ~deref ~pure (Path { p_update; _ }) = p_update ~deref ~pure

      let deref : type a. a Type.t -> (a addr, a) path =
       fun codec ->
        let open Addr in
        let ( >|= ) = Lwt.Infix.( >|= ) in
        let p_project_type = function
          | Boxed_typ (Typ_addr a) -> Boxed_typ a
          | Boxed_typ _ -> assert false
          (* The source type is definitely an [Addr.t]. *)
        in
        let p_get ~deref:{ deref } (Addr a_ref) = deref a_ref in
        let p_update ~deref:_ ~pure:{ pure } _ref a =
          pure codec a >|= fun a -> Addr a
        in
        Path { p_project_type; p_steps = []; p_get; p_update }

      let steps _ = assert false
      ...
      let ( / ) (Path outer) (Path inner) =
        ...
        in
        let p_get ~deref s =
          outer.p_get ~deref s >>= function
          | Some mid -> inner.p_get ~deref mid
          | None -> Lwt.return None
        in
        let p_update ~deref ~pure s a =
          outer.p_get ~deref s >>= function
          | None -> Lwt.return s
          | Some mid ->
              inner.p_update ~deref ~pure mid a >>= fun mid ->
              outer.p_update ~deref ~pure s mid
        in
        Path { p_steps; p_project_type; p_get; p_update }

      let empty =
        let p_get ~deref:_ s = Lwt.return (Some s) in
        let p_update ~deref:_ ~pure:_ _ s = Lwt.return s in
    ...
    module Record = struct
      ...
      let ( |+ ) :
       ...
       fun { open_record = previous } field ->
        let open_record' :
            type hole.
            (r, rem, paths_nil, hole) fields ->
            (r, c, paths, hole) Unwitnessed.t =
      ...
      let sealr :
       ...
       fun { open_record = r } r_type ->
        ...
        let paths =
          let rec inner :
              type a l. (record_t, a, l, unit) fields -> l Path.hlist = function
            | Fields_nil -> []
            | Fields_cons (Field { f_get; f_update; f_type; f_name }, fs) ->
                let p_steps = [ f_name ] in
                let p_project_type _ = Boxed_typ f_type in
                let p_get ~deref:_ s = Lwt.return (Some (f_get s)) in
                let p_update ~deref:_ ~pure:_ s a = Lwt.return (f_update s a) in
    ...
    module Variant = struct
      ...
      let rec add_cases_to_destructor :
       ...
       fun p -> function
        ...
        | Cases_cons (Case case, cs) ->
            add_cases_to_destructor
              (p (fun v -> Case_typeable (Case case, v)))
              cs
      ...
      (** Cases are accumulated while their tags remain unknown. Tags are
          assigned when the variant is sealed. *)
      ...
      let preview :
          type v c.
          (v -> v case_typeable) -> c Witness.t -> int -> v -> c option =
      ...
      let paths_of_cases (type variant)
          (v_get : variant -> variant case_typeable) =
        let rec inner :
            type a b paths.
            (variant, a, b, paths, unit) cases -> paths Path.hlist = function
          | Cases_nil -> []
          | Cases_cons
              ( Case
                  {
              ...
              let p_project_type _ = Boxed_typ c_type in
              let p_update ~deref:_ ~pure:_ s a =
                let (Case_typeable (Case { c_tag = tag_actual; _ }, _)) =
                  v_get s
                in
                ( if Int.equal tag_actual tag_expected then c_constructor a
                else s )
                |> Lwt.return
              in
              let p_get ~deref:_ s =
